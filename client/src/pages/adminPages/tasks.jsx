...
const UserDetails = () => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/view-user-tasks/${id}`, {
        headers: {
  ...
  const handleDeleteTask = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/delete-user-task/${selectedTask._id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleUpdateTask = async (values) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/update-user-task/${selectedTask._id}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-4">User Tasks</h2>
      <Table
        dataSource={tasks}
        columns={columns}
        rowKey="_id"
